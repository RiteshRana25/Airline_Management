...
const CalendarDiv5 = () => {
  ...
  const departureDate = departureDateString3
    ? new Date(departureDateString3)
    : null;

  const [isCalendarOpen, setIsCalendarOpen] = useState(false);
  const [selectedDate, setSelectedDate] = useState(
    departureDateString4 ? new Date(departureDateString4) : null
  );
  const [currentMonth, setCurrentMonth] = useState(
    departureDateString4 ? new Date(departureDateString4) : new Date()
  );
  ...
  const handleDayClick = (day) => {
    ...
    if (
      ...
      setDepartureDateString4(formatDate(date)); // Store the formatted date in context
  ...
  useEffect(() => {
    if (departureDateString3) {
      const newDepartureDate = new Date(departureDateString3);
      const currentSelectedDate = selectedDate || new Date(departureDateString4);

      if (newDepartureDate > currentSelectedDate) {
        setSelectedDate(newDepartureDate);
        setDepartureDateString4(formatDate(newDepartureDate));
      }
    }
  }, [departureDateString3, departureDateString4, setDepartureDateString4, selectedDate]);
