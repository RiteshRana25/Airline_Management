...
const LocationSearchMulti4 = ({
  ...
  const handleSelectFrom = (airport) => {
    if (airport.address.cityCode === selectedTo4.code) {
      ...
      setError_5("From and To airports cannot be the same.");
      return;
    }
    ...
    setResultsFrom([]);
    setError_5(""); // Clear error if selection is valid
  };
  ...
  return (
    <div className="location-container-multi">
      ...
      >
        ...
        >
          {selectedFrom4.code !== "" ? <>{selectedFrom4.city}
          {" , "}
          {selectedFrom4.country}</> : ""}
        </span>
      </div>
      ...
      <div
        ...
        onClick={() => {
          ...
          setSelectedTo4(temp);
          setError_5(""); // Clear error when swapping
        }}
      >
        <SwapHorizIcon />
      </div>
      ...
      <div className="to-multi" ref={toRef} onClick={handleToLocationClick}>
        ...
        >
          {" "}
          {selectedTo4.code !== "" ? <>{selectedTo4.city}
          {" , "}
          {selectedTo4.country}</> : ""}
        </span>
      </div>
...
export default LocationSearchMulti4;
