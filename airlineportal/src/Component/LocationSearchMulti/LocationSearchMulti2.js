...
const LocationSearchMulti2 = ({
  ...
  const handleSelectFrom = (airport) => {
    if (airport.address.cityCode === selectedTo2.code) {
      ...
      setError_3("From and To airports cannot be the same.");
      return;
    }
    ...
    setResultsFrom([]);
    setError_3(""); // Clear error if selection is valid
  };
  ...
  return (
    <div className="location-container-multi">
      ...
      >
        ...
        >
          {selectedFrom2.code !== "" ? <>{selectedFrom2.city}
          {" , "}
          {selectedFrom2.country}</> : ""}
        </span>
      </div>
      ...
      <div
        ...
        onClick={() => {
          ...
          setSelectedTo2(temp);
          setError_3(""); // Clear error when swapping
        }}
      >
        <SwapHorizIcon />
      </div>
      ...
      <div className="to-multi" ref={toRef} onClick={handleToLocationClick}>
        ...
        >
          {selectedTo2.code !== "" ? <>{selectedTo2.city}
          {" , "}
          {selectedTo2.country}</> : ""}
        </span>
      </div>
...
export default LocationSearchMulti2;
