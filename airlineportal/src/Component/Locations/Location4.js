...
const Location4 = ({
  ...
  const fetchAirportData = async (query, type) => {
    ...
    try {
      ...
      setError6(error.response?.data?.message || "Error fetching airport data");
    }
  };
  ...
  const handleSelectFrom = (airport) => {
    if (airport.address.cityCode === selectedTo4.code) {
      alert("From and To airports cannot be the same.")
      setError6("From and To airports cannot be the same.");
      return;
    }
    ...
    setResultsFrom([]);
    setError6(""); // Clear error if selection is valid
  };

  const handleSelectTo = (airport) => {
    if (airport.address.cityCode === selectedFrom4.code) {
      alert("From and To airports cannot be the same.")
      setError6("From and To airports cannot be the same.");
      return;
    }
    ...
    <div className="location-container">
      ...
      <div className="from" ref={fromRef} onClick={handleFromLocationClick}>
        ...
        >
          {selectedFrom4.code !== "" ? (
            <>
              {selectedFrom4.code}, {selectedFrom4.name}
            </>
          ) : (
            ""
          )}
      ...
      <div
        className="swap"
        onClick={() => {
          const temp = selectedFrom4;
          setSelectedFrom4(selectedTo4);
          setSelectedTo4(temp);
          setError6(""); // Clear error when swapping
        }}
      >
        <SwapHorizIcon />
      </div>
      ...
      <div className="to" ref={toRef} onClick={handleToLocationClick}>
        ...
        >
          {selectedTo4.code !== "" ? <>{selectedTo4.city}</> : "Select City"}
        </span>
        <span
          className="tofrom"
          style={{
        ...
        >
          {selectedTo4.code !== "" ? (
            <>
              {selectedTo4.code}, {selectedTo4.name}
            </>
          ) : (
            ""
          )}
