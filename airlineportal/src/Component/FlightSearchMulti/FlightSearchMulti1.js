...
const FlightSearchMulti1 = ({
    ...
    try {
      ...
      const flightResponse = await axios.get(
        "https://test.api.amadeus.com/v2/shopping/flight-offers",
        {
          ...
          params: {
            originLocationCode: selectedFrom1.code,
            destinationLocationCode: selectedTo1.code,
            departureDate: departureDateString1,
            adults: 1,
      ...
      // One-stop or more-than-one-stop filtering
      if (isOneStop) {
        flightsData = flightsData.filter((flight) =>
      ...
      setError02(err.response?.data?.message || "Error fetching flight data");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchFlightData();
    setCityName1(selectedFrom1.city);
    setCityName2(selectedTo1.city);
    setRun1(false);
  }, [
    run1,
    nonStop,
  ...
  const handleSelectFlight = (flight, itinerary) => {
    const date = new Date(departureDateString1);
    setFormattedDate1(formatDate(date));
    ...
    setSelectedFlightDetails1(flightDetails);
  };
  return (
    ...
    >
      <h2>
        Select flight From {selectedFrom1.city} to {selectedTo1.city}
      </h2>

      {error02 && <p style={{ color: "red" }}>{error02}</p>}
...
export default FlightSearchMulti1;
