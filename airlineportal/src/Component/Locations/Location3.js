...
const Location3 = ({
  ...
  const fetchAirportData = async (query, type) => {
    ...
    try {
      ...
      setError7(error.response?.data?.message || "Error fetching airport data");
    }
  };
  ...
  const handleSelectFrom = (airport) => {
    if (airport.address.cityCode === selectedTo3.code) {
      ...
      setError7("From and To airports cannot be the same.");
      return;
    }
    ...
    setResultsFrom([]);
    setError7(""); // Clear error if selection is valid
  };
    ...
    <div className="location-container">
      ...
      <div className="from" ref={fromRef} onClick={handleFromLocationClick}>
        ...
        >
          ...
          {selectedFrom3.code !== "" ? (
            <>
              {selectedFrom3.code}, {selectedFrom3.name}
            </>
          ) : (
            ""
          )}
      ...
      <div
        ...
        onClick={() => {
          ...
          setSelectedTo3(temp);
          setError7(""); // Clear error when swapping
        }}
      >
        <SwapHorizIcon />
      </div>
      ...
      <div className="to" ref={toRef} onClick={handleToLocationClick}>
        ...
        >
          {selectedTo3.code !== "" ? <>{selectedTo3.city}</> : "Select City"}
        </span>
        <span
          className="tofrom"
          style={{
        ...
        >
          ...
          {selectedTo3.code !== "" ? (
            <>
              {selectedTo3.code}, {selectedTo3.name}
            </>
          ) : (
            ""
          )}
