...
const FlightSearchMulti4 = ({
    ...
    try {
      ...
      const flightResponse = await axios.get(
        "https://test.api.amadeus.com/v2/shopping/flight-offers",
        {
          ...
          params: {
            originLocationCode: selectedFrom4.code,
            destinationLocationCode: selectedTo4.code,
            departureDate: departureDateString4,
            adults: 1,
      ...
      setError05(err.response?.data?.message || "Error fetching flight data");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchFlightData();
    setCityName1(selectedFrom4.city);
    setCityName2(selectedTo4.city);
    setRun1(false);
  }, [
    run1,
    nonStop,
  ...
  const handleSelectFlight = (flight, itinerary) => {
    const date = new Date(departureDateString4);
    setFormattedDate4(formatDate(date));
    setSelectedFlight4(flight);
  ...
  return (
    ...
    >
      <h2>
        Select flight From {selectedFrom4.city} to {selectedTo4.city}
      </h2>

      {error05 && <p style={{ color: "red" }}>{error05}</p>}
...
export default FlightSearchMulti4;
