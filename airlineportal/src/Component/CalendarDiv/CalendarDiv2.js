...
const CalendarDiv2 = () => {
  ...
  const [selectedDate, setSelectedDate] = useState(
    departureDateString1 ? new Date(departureDateString1) : null
  );
  const [currentMonth, setCurrentMonth] = useState(
    departureDateString1 ? new Date(departureDateString1) : new Date()
  );
  ...
  const handleDayClick = (day) => {
    ...
    if (
      ...
      setDepartureDateString1(formatDate(date)); // Store the formatted date in context
  ...
  useEffect(() => {
    if (departureDateString) {
      ...
      const currentSelectedDate = selectedDate || new Date(departureDateString1);

      if (newDepartureDate > currentSelectedDate) {
        setSelectedDate(newDepartureDate);
        setDepartureDateString1(formatDate(newDepartureDate));
      }
    }
  }, [departureDateString, departureDateString1, setDepartureDateString1, selectedDate]);
