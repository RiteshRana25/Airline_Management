...
const apiSecret = process.env.REACT_APP_API_SECRET;
  ...
  const fetchAirportData = async (query, type) => {
    ...
    try {
      ...
      setError9(error.response?.data?.message || "Error fetching airport data");
    }
  };
  ...
  const handleSelectFrom = (airport) => {
    if (airport.address.cityCode === selectedTo1.code) {
      alert("From and To airports cannot be the same.")
      setError9("From and To airports cannot be the same.");
      return;
    }
    ...
    setResultsFrom([]);
    setError9(""); // Clear error if selection is valid
  };

  const handleSelectTo = (airport) => {
    if (airport.address.cityCode === selectedFrom1.code) {
      alert("From and To airports cannot be the same.")
      setError9("From and To airports cannot be the same.");
      return;
    }
    ...
    <div className="location-container">
      ...
      <div className="from" ref={fromRef} onClick={handleFromLocationClick}>
        ...
        >
          {selectedFrom1.code !== "" ? (
            <>
              {selectedFrom1.code}, {selectedFrom1.name}
            </>
          ) : (
            ""
          )}
      ...
      <div
        className="swap"
        onClick={() => {
          const temp = selectedFrom1;
          setSelectedFrom1(selectedTo1);
          setSelectedTo1(temp);
          setError9(""); // Clear error when swapping
        }}
      >
        <SwapHorizIcon />
      </div>
      ...
      <div className="to" ref={toRef} onClick={handleToLocationClick}>
        ...
        >
          {selectedTo1.code !== "" ? <>{selectedTo1.city}</> : "Select City"}
        </span>
        ...
        >
          {selectedTo1.code !== "" ? (
            <>
              {selectedTo1.code}, {selectedTo1.name}
            </>
          ) : (
            ""
          )}
