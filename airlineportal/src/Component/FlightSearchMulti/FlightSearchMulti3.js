...
const apiSecret = process.env.REACT_APP_API_SECRET;
    ...
    try {
      ...
      const flightResponse = await axios.get(
        "https://test.api.amadeus.com/v2/shopping/flight-offers",
        {
          ...
          params: {
            originLocationCode: selectedFrom3.code,
            destinationLocationCode: selectedTo3.code,
            departureDate: departureDateString3,
            adults: 1,
      ...
      setError04(err.response?.data?.message || "Error fetching flight data");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchFlightData();
    setCityName1(selectedFrom3.city);
    setCityName2(selectedTo3.city);
    setRun1(false);
  }, [
    run1,
    nonStop,
  ...
  const handleSelectFlight = (flight, itinerary) => {
    const date = new Date(departureDateString3);
    setFormattedDate3(formatDate(date));
    setSelectedFlight3(flight);
    ...
    setSelectedFlightDetails3(flightDetails);
  };
  return (
    ...
    >
      <h2>
        Select flight From {selectedFrom3.city} to {selectedTo3.city}
      </h2>

      {error04 && <p style={{ color: "red" }}>{error04}</p>}
...
export default FlightSearchMulti3;
