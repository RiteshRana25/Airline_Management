...
const CalendarDiv4 = () => {
  ...
  const departureDate = departureDateString2
    ? new Date(departureDateString2)
    : null;

  const [isCalendarOpen, setIsCalendarOpen] = useState(false);
  const [selectedDate, setSelectedDate] = useState(
    departureDateString3 ? new Date(departureDateString3) : null
  );
  const [currentMonth, setCurrentMonth] = useState(
    departureDateString3 ? new Date(departureDateString3) : new Date()
  );
  ...
  const handleDayClick = (day) => {
    ...
    if (
      ...
      setDepartureDateString3(formatDate(date)); // Store the formatted date in context
  ...
  useEffect(() => {
    if (departureDateString2) {
      const newDepartureDate = new Date(departureDateString2);
      const currentSelectedDate = selectedDate || new Date(departureDateString3);

      if (newDepartureDate > currentSelectedDate) {
        setSelectedDate(newDepartureDate);
        setDepartureDateString3(formatDate(newDepartureDate));
      }
    }
  }, [departureDateString2, departureDateString3, setDepartureDateString3, selectedDate]);
