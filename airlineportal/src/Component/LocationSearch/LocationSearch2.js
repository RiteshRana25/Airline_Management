...
const apiSecret = process.env.REACT_APP_API_SECRET;
  ...
  const fetchAirportData = async (query, type) => {
    ...
    try {
      ...
      // Clear errors if data is fetched successfully
      setError3("");
    } catch (error) {
      setError3(error.response?.data?.message || "Error fetching airport data");
    }
  };
  ...
  const handleSelectFrom = (airport) => {
    if (airport.address.cityCode === selectedTo.code) {
      ...
      setError4("From and To airports cannot be the same.");
      return;
    }
    ...
    setResultsFrom([]);
    setError4(""); // Clear error if selection is valid
  };
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              padding: "5px", // Optional: Add padding for spacing // Optional: Set a background color
